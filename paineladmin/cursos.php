...
<head>
    ...
    <style>
        ...
        :root {
            ...
            --border-color: rgba(255, 255, 255, 0.1); --success-color: #22c55e; --info-color: #3b82f6; --delete-color: #ef4444;
        }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* === ESTILOS ESPECÍFICOS da cursos.php === */
        ...
        .course-card { background: var(--glass-background); border: 1px solid var(--border-color); border-radius: 12px; overflow: hidden; display: flex; flex-direction: column; transition: transform 0.2s, border-color 0.2s; }
        ...
        .course-card-stats { display: flex; flex-wrap: wrap; gap: 1.5rem; color: var(--text-muted); font-size: 0.9rem; margin: 1rem 0 1.5rem 0; }
        .course-card-stats span { display: inline-flex; align-items: center; gap: 0.25rem; }
        .course-card-stats svg { width:16px; height:16px; }
        .course-card-actions { display: flex; gap: 1rem; margin-top: auto; }
        .course-card-actions .btn-secondary { padding: 0.75rem; line-height: 1; }
        .course-card-actions .btn-secondary svg { width:20px; height:20px; color: var(--text-muted); }
        .course-card-actions .btn-secondary:hover svg { color: var(--primary-color); }
        ...
        .lesson-item-info { flex-grow: 1; overflow: hidden; }
        .lesson-item-title { font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .lesson-item-desc { font-size: 0.85rem; color: var(--text-muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .actions-cell { display: flex; gap: 0.5rem; align-items: center; flex-shrink: 0; }
        .actions-cell button, .actions-cell a { background: none; border: none; cursor: pointer; padding: 0.5rem; line-height: 1; display: inline-flex; align-items: center; justify-content: center; border-radius: 4px; }
        .actions-cell button svg, .actions-cell a svg { width: 20px; height: 20px; color: var(--text-muted); transition: color 0.2s; }
        ...
        .lesson-item-files { font-size: 0.8em; color: var(--text-muted); margin-top: 4px; cursor: pointer; display: inline-flex; align-items: center; }
        .lesson-item-files span { color: var(--success-color); font-weight: 600; margin-left: 5px; }
        .lesson-item-files svg { width: 16px; height: 16px; color: var(--success-color); }
        ...
        .form-group input, .form-group textarea { width: 100%; padding: 0.75rem; background-color: var(--background-color); border: 1px solid var(--border-color); border-radius: 8px; color: var(--text-color); font-size: 1rem; font-family: 'Poppins', sans-serif; transition: border-color 0.2s; }
        ...
        .file-entry-info { flex-grow: 1; overflow: hidden; }
...
<body>
    ...
    <main class="main-content">
        ...
        <?php elseif ($view === 'lessons' && !empty($curso_ativo)): ?>
            <header class="lesson-manager-header">
                <a href="cursos.php" class="btn-secondary" style="padding: 0.75rem;">&larr; Voltar</a>
                <div>
                    <h1 style="font-size: 1.8rem;"><?php echo htmlspecialchars($curso_ativo['titulo']); ?></h1>
                    <p style="color: var(--text-muted);">Gerencie as aulas e materiais deste curso.</p>
                </div>
            </header>
            <?php if ($feedback_message): ?><div class="feedback-message <?php echo $feedback_type; ?>"><?php echo htmlspecialchars($feedback_message); ?></div><?php endif; ?>
            ...
            <ul id="lesson-list" class="lesson-list">
                ...
                <li class="lesson-item" data-id="<?php echo $aula['id']; ?>">
                    ...
                    <div class="lesson-item-info">
                        ...
                        <?php $file_count = count($arquivos_por_aula[$aula['id']] ?? []); ?>
    ...
    <div class="modal-overlay" id="file-modal">
        <div class="modal-content">
            <div class="modal-header"> <h2 id="file-modal-title">Adicionar Arquivo à Aula</h2> <button type="button" class="close-modal">&times;</button> </div>
            <form id="file-form" method="POST">
                ...
                <div class="form-group"> <label for="file-titulo">Título do Arquivo</label> <input type="text" id="file-titulo" name="file_titulo" required> </div>
                <div class="form-group"> <label for="file-descricao">Descrição (Opcional)</label> <textarea id="file-descricao" name="file_descricao"></textarea> </div>
                <div class="form-group"> <label for="file-caminho">URL do Arquivo (Link do Drive, S3, Dropbox, etc.)</label> <input type="url" id="file-caminho" name="file_caminho" required placeholder="https://drive.google.com/link_do_arquivo"> </div>
                <div class="modal-footer"> <button type="submit" class="btn-primary" id="file-modal-submit">Salvar Arquivo</button> </div>
            </form>
        </div>
    </div>
    ...
    <div class="modal-overlay" id="file-list-modal">
        <div class="modal-content">
            <div class="modal-header"> <h2 id="file-list-modal-title">Arquivos da Aula</h2> <button type="button" class="close-modal">&times;</button> </div>
            <div id="file-list-content"> </div>
            <div class="modal-footer"> <button type="button" class="btn-primary" id="add-new-file-from-list">+ Adicionar Novo Arquivo</button> </div>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // --- Gerenciamento de Cursos (Grid View) ---
        ...
        // --- Gerenciamento de Aulas (Lessons View) ---
        const activeCourseId = document.getElementById('curso_id_aula')?.value;
        if (activeCourseId) {
            // Função para abrir modal de Add/Edit Arquivo
            ...
            document.querySelectorAll('.edit-lesson-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    ...
                    document.getElementById('curso_id_aula').value = aula.curso_id;
            ...
            // Abrir modal de Add Arquivo (pelo botão + da aula)
            ...
            // Abrir modal de LISTA de Arquivos
            ...
            // Abrir modal de Edição de Arquivo (de dentro da lista)
            ...
            // Abrir modal de Add Arquivo (de dentro da lista)
