...
$pagina_atual = basename($_SERVER['PHP_SELF']); // Necessário para o sidebar
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* === 1. SIDEBAR (Base Desktop) === */
        .sidebar {
            width: 260px; background-color: var(--sidebar-color); height: 100vh;
            ...
            transition: transform 0.3s ease; /* Para animação mobile */
        }
        ...
        /* Dropdown e Perfil (Reutilizado do código original) */
        ...
        /* === 2. MAIN CONTENT (Base Desktop) === */
        .main-content {
            margin-left: 260px; flex-grow: 1; padding: 2rem 3rem;
        ...
        .menu-toggle { display: none; /* Escondido no desktop */
            position: fixed; top: 1.5rem; left: 1.5rem; z-index: 1001; cursor: pointer;
            padding: 10px; background-color: var(--sidebar-color); border-radius: 8px;
            border: 1px solid var(--border-color);
        }
        .menu-toggle svg { width: 24px; height: 24px; color: var(--text-color); }


        /* === 3. ESTILOS DE DASHBOARD (Mantido) === */
        ...
        .stat-card .icon-wrapper { width: 60px; height: 60px; border-radius: 50%; display: flex; align-items: center; justify-content: center; background: rgba(225, 29, 72, 0.1); border: 1px solid rgba(225, 29, 72, 0.3); }
        .stat-card .icon-wrapper svg { width: 28px; height: 28px; color: var(--primary-color); }
        .stat-info .stat-number { font-size: 2rem; font-weight: 700; }
        .stat-info .stat-label { font-size: 0.9rem; color: var(--text-muted); }

        .chart-container { background: var(--glass-background); padding: 2rem; border-radius: 12px; border: 1px solid var(--border-color); height: 400px; /* Altura fixa para o gráfico */ }
        ...
        .info-panel h3 svg { color: var(--primary-color); }
        ...
        .info-panel .item-main .item-title { font-weight: 500; font-size: 0.9rem; }
        .info-panel .item-main .item-subtitle { font-size: 0.8rem; color: var(--text-muted); }
        .info-panel .item-action a { font-size: 0.8rem; color: var(--primary-color); text-decoration: none; font-weight: 500; }

        /* === 4. RESPONSIVIDADE (Mobile/Tablet) === */
        @media (max-width: 1024px) {
            .sidebar {
                width: 280px;
                transform: translateX(-280px); /* Esconde o sidebar fora da tela */
            ...
            .main-content {
                ...
                width: 100%;
                padding: 1.5rem;
                padding-top: 5rem; /* Espaço para o botão menu-toggle */
            }
            .menu-toggle {
                display: flex; /* Mostra o botão no mobile/tablet */
            }

            /* Classe dinâmica para abrir/fechar o menu */
            ...
            body.sidebar-open::after {
                ...
                z-index: 1001; /* Fundo escuro para fechar o sidebar */
            }

            .chart-container {
                height: 350px; /* Ajuste a altura do gráfico para telas menores */
            }
        }
...
<body>
    <?php
    // Inclui o sidebar agora de um arquivo externo
        ...
        <section class="stats-grid">
            ...
            <div class="stat-card">
                <div class="icon-wrapper">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.91 11.672a.375.375 0 010 .656l-5.603 3.113a.375.375 0 01-.557-.328V8.887c0-.286.307-.466.557-.327l5.603 3.112z" /></svg>
                </div>
    ...
    <script>
        // === LÓGICA JS DO SIDEBAR (Hamburger e Dropdown) ===
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (menuToggle && sidebar) {
                // Abre/Fecha o sidebar ao clicar no ícone
                menuToggle.addEventListener('click', (event) => {
                    event.stopPropagation();
                    body.classList.toggle('sidebar-open');
                });

                // Fecha o sidebar ao clicar fora (no overlay)
                body.addEventListener('click', (event) => {
                    // Verifica se o menu está aberto E o clique não foi no menu-toggle ou no próprio sidebar
                    if (body.classList.contains('sidebar-open') && !sidebar.contains(event.target) && !menuToggle.contains(event.target)) {
                        body.classList.remove('sidebar-open');
                    }
                });

                // Fecha o sidebar ao clicar em um link (para mobile)
